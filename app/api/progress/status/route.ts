...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => null) as { sessionId?: string; learningStatus?: string } | null
  if (!body?.sessionId || !body.learningStatus || !allowedStatuses.has(body.learningStatus)) {
    return NextResponse.json({ error: 'invalid_learning_status_payload' }, { status: 400 })
  }

  const { data, error } = await supabase.rpc('set_material_learning_status', {
  ...
  revalidatePath(`/portal/session/${body.sessionId}`)
